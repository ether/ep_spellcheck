# Contributing to ep_spellcheck

Thanks for helping improve `ep_spellcheck` — a plugin for [Etherpad](https://etherpad.org/). LLM/Agent contributions are explicitly welcome, provided they follow the rules below.
...
`ep_spellcheck` is a plugin, so you develop it inside a checkout of [ether/etherpad](https://github.com/ether/etherpad).
...
# Install ep_spellcheck from your local clone
pnpm run plugins i --path ../ep_spellcheck

# Start the dev server (port 9001)
pnpm --filter ep_etherpad-lite run dev
```

Edits in `../ep_spellcheck/` are picked up after a server restart.
...
* Etherpad version, ep_spellcheck version, browser + OS
* Steps to reproduce
